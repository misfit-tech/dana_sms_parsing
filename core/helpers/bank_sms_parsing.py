...
raw_data = '''{
"data": "{\"address\":[\"16216\",\"16216\",\"16216\",\"16216\"],\"body\":[\"Dear Sir, your DBBL A\/C **9788 credited by BDT 45,000.00 on 08-07-2020 09:48:57 AM.Enjoy shopping with your Debit Card.Call 16216\",\"Dear Sir, your DBBL A\/C **6654 credited (Interest Credit) by BDT 1.98 on 01-07-2020 04:15:24 AM.Please Download NexusPay https:\/\/bit.ly\/nexuspay .Call 16216\",\"Tk2000.00 received from A\/C:102010201 Fee:Tk0, Your A\/C Balance: Tk3623.87 TxnId:1829598802 Date:13-JUL-20 01:27:27 pm. Download https:\/\/bit.ly\/nexuspay\",\"Cash-Out from ATM Tk10,00.00Fee:Tk Your A\/C Balance: Tk15.87.TxnId:1841272889Date:19-JUL-20 08:18:39 am. Please download https:\/\/bit.ly\/nexuspay\"],\"customer_id\":[\"01677079251\",\"01677079251\",\"01677079251\",\"01677079251\"],\"date\":[\"1598346966300\",\"1598346153243\",\"1598339524891\",\"1598333712566\"],\"id\":[\"17241\",\"17241\",\"17241\",\"17241\"]}"
}'''
...
# convert date integer to actual date
# date = datetime.datetime.fromtimestamp(dataframe['date']/1e3)
# filter for date for the previous 1 month
# for date<== today & date>== timedelta(today-1 month)
# filter for individual user
# for id ==
# filter for address
# for address == 
# dataframe.aggregate()
# sum of all fields (except 'balance')
# latest value of balance (for each individual address)
# balance = dataframe['balance'].tail(1).values
...
# json_string = data.reset_index().to_json(orient='records')

import datetime
your_timestamp = 1598333712566
...
# convert datetime integer to datetime stamp
#data['date'] = data['date'].astype(int) 
# data['date'] = datetime.datetime.fromtimestamp(data['date'].astype(float) / 1e3) 
#data['date'] = pd.to_datetime(data['date'], unit='s') 
 
# check
#data['date']
 
#df_unix_sec = pd.to_datetime(data['date']).astype(int)/ 10**9
#print(df_unix_sec)
...
data = data.groupby(['id', 'address']).agg({'debit': 'sum', 'credit': 'sum', 'withdrawal': 'sum', 'card_payment': 'sum', 'card_purchase': 'sum', 'interest_credit': 'sum', 'deposit': 'sum', 'bank_to_mobile_recharge': 'sum', 'balance': 'last', 'fund_transfer_in': 'sum', 'fund_transfer_out': 'sum'}).reset_index()
